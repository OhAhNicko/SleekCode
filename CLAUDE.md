...
## Project
- Product: MADE — AI terminal workspace (Warp/BridgeSpace-inspired)
...
## Updating the MADE voice server (Mac mini)

The voice agent (push-to-talk + intent → action) calls a small FastAPI service on the user's Mac mini. The server is **separate from** the podcast app's `podd-ad-server` and lives at `~/projects/made-voice-server/server.py` on the Mac mini, listening on **port 8770**. It exposes:
...
1. **Edit local working copy** at `/tmp/made_voice_server.py` using Edit/Write tools. Never edit the Mac mini's `server.py` directly.
2. **Syntax-check**:
   ```bash
   python3 -c "import ast; ast.parse(open('/tmp/made_voice_server.py').read()); print('syntax ok')"
   ```
3. **Upload to temp.sh**:
   ```bash
   curl -sF "file=@/tmp/made_voice_server.py" "https://temp.sh/upload"
   ```
   Returns a URL like `https://temp.sh/ABCDE/made_voice_server.py`.
...
- **Isolated venv at `~/projects/made-voice-server/.venv/`.** Do NOT share the podcast server's venv — that re-couples the two projects. First deploy installs `mlx-whisper` into the MADE venv (~3 min, ~2 GB disk); subsequent deploys reuse it instantly.
...
- **Verify from the Windows machine** after deploy: `curl http://<mac-mini-tailscale>:8770/health` must succeed before MADE's "Test" buttons will go green.
- **MADE settings** after a successful deploy:
...
- **Streaming is disabled in MADE's LLM client** (`stream: false`), so the chat-completions handler is a single forward-and-return. Don't add SSE complexity unless MADE's client changes.
- **Do NOT extend `podd-ad-server/server.py`** to host MADE endpoints. The two services stay isolated so podcast-server changes can never break voice and vice versa.
